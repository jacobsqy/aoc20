...
fn part2(input: &str) -> u32 {
    let mut active_cubes: Vec<(i32, i32, i32, i32)> = Vec::new();
    for (x, line) in input.lines().enumerate() {
        for (y, c) in line.chars().enumerate() {
            if c == '#' {
                active_cubes.push((x as i32, y as i32, 0, 0));
            }
        }
    }

    for _ in 0..6 {
        let mut new_active_cubes: Vec<(i32, i32, i32, i32)> = Vec::new();
        let mut neighbours: HashMap<(i32, i32, i32, i32), u32> = HashMap::new();
        for cube in &active_cubes {
            for neighbour in find_4d_neighbours(cube) {
                neighbours
                    .entry(neighbour)
                    .and_modify(|n| *n += 1)
                    .or_insert(1);
...
fn find_4d_neighbours((x, y, z, w): &(i32, i32, i32, i32)) -> Vec<(i32, i32, i32, i32)> {
    let mut result = Vec::new();
    for x_alt in x - 1..=x + 1 {
        for y_alt in y - 1..=y + 1 {
            for z_alt in z - 1..=z + 1 {
                for w_alt in w - 1..=w + 1 {
                    if x_alt != *x || y_alt != *y || z_alt != *z || w_alt != *w {
                        result.push((x_alt, y_alt, z_alt, w_alt));
                    }
                }
            }
        }
